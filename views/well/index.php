...
<div class="wd-main">
  ...
  <form method="get" action="<?php echo h($baseUrl); ?>" class="wd-search">
    <div class="wd-grid">
      <div>
        <label>UWI</label>
        <input type="text" name="uwi" value="<?php echo h($filters['uwi']); ?>" placeholder="Ej: 007WHZPZ0060 1">
      </div>
      <div>
    ...
    <div class="wd-actions">
      ...
      <div>
        ...
        <button type="button" class="wd-btn wd-btn-clear" onclick="window.location.href='<?php echo h($baseUrl); ?>'">Limpiar</button>
      </div>
    </div>
  </form>
      ...
      <tbody>
        <?php if (!empty($rows)): ?>
          ...
          <tr>
            ...
            <td>
                <a class="btn" href="<?php echo h(BASE_PATH . '/well/details/' . urlencode($r['UWI'] ?? '')); ?>">Ver</a>
                <a class="btn" href="<?php echo h(BASE_PATH . '/well/edit/' . urlencode($r['UWI'] ?? '')); ?>">Editar</a>
            </td>
          </tr>
          <?php endforeach; ?>
        <?php else: ?>
